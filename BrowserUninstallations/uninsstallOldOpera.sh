...
# Check version from first valid app
PLIST="${OPERA_PATHS[0]}/Contents/Info.plist"
INSTALLED_VERSION=$(/usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" "$PLIST" 2>/dev/null)

if [ -z "$INSTALLED_VERSION" ]; then
    echo "Could not determine Opera version. Proceeding with uninstall for safety."
else
    echo "Installed version: $INSTALLED_VERSION"
    echo "Target version: $TARGET_VERSION"

    if ! version_lt "$INSTALLED_VERSION" "$TARGET_VERSION"; then
        echo "Opera is up-to-date. Exiting."
        exit 0
    fi
fi
